...
if ($tab === 'students') {
    $result = $conn->query("SELECT `acc_no`, name, grade, contact, email, address, role, profile_picture FROM account_record WHERE role = 'student' ORDER BY name");
    $students_with_pics = $result->fetch_all(MYSQLI_ASSOC);
    $students = $students_with_pics;
} elseif ($tab === 'teachers') {
    $result = $conn->query("SELECT `acc_no`, name, grade, contact, email, address, role, profile_picture FROM account_record WHERE role = 'teacher' ORDER BY name");
    $teachers_with_pics = $result->fetch_all(MYSQLI_ASSOC);
    $teachers = $teachers_with_pics;
} elseif ($tab === 'admins') {
    $result = $conn->query("SELECT `acc_no`, name, grade, contact, email, address, role FROM account_record WHERE role = 'admin' ORDER BY name");
    ...
    $result = $conn->query("
        SELECT se.enrollment_id, se.class_id, se.enrolled_at, se.completed, se.completed_at,
               c.class_name, ar.name, ar.email, ar.grade,
               COUNT(stp.progress_id) as total_tasks,
               SUM(CASE WHEN stp.completed = 1 THEN 1 ELSE 0 END) as completed_tasks
        FROM student_enrollments se
        LEFT JOIN classes c ON se.class_id = c.class_id
        LEFT JOIN account_record ar ON se.student_id = ar.`acc_no`
        LEFT JOIN student_task_progress stp ON se.student_id = stp.student_id
...
<body>
    ...
    <div class="container">
        ...
        <div class="tabs">
            ...
            <button class="tab-btn <?php echo $tab === 'enrollments' ? 'active' : ''; ?>" onclick="location.href='?tab=enrollments'">Enrollments</button>
        </div>
            ...
            <table>
                ...
                <tbody>
                    <?php if (count($teachers) > 0): ?>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="7" class="no-data">No teachers found</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
                ...
                <tbody>
                    <?php if (count($enrollments) > 0): ?>
                        ...
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($enrollment['class_name'] ?? 'N/A'); ?></td>
                            <td><?php echo htmlspecialchars($enrollment['email'] ?? 'N/A'); ?></td>
                            <td><?php echo htmlspecialchars($enrollment['grade'] ?? 'N/A'); ?></td>
                            <td>
                                <div class="progress-bar">
                                    <?php 
                                        $percentage = $enrollment['total_tasks'] > 0 ? round(($enrollment['completed_tasks'] / $enrollment['total_tasks']) * 100) : 0;
                                    ?>
                                    <div class="progress-fill" style="width: <?php echo $percentage; ?>%">
                                        <?php echo $enrollment['completed_tasks'] ?? 0; ?>/<?php echo $enrollment['total_tasks'] ?? 0; ?>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <span class="status-badge <?php echo $enrollment['completed'] ? 'status-completed' : 'status-active'; ?>">
                                    <?php echo $enrollment['completed'] ? 'Completed' : 'In Progress'; ?>
                                </span>
                            </td>

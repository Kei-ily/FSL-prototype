...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'add_task') {
    ...
    if (!empty($task_name)) {
        $insert_query = "INSERT INTO class_tasks (class_id, task_name, task_description, file_path, file_name) VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($insert_query);
        $stmt->bind_param("issss", $class_id, $task_name, $task_description, $task_file, $task_file_name);
        $stmt->execute();
        $stmt->close();
    }
}
